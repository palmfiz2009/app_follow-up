...
# --- セッション状態初期化 ---
if 'init_followup_clean_done' not in st.session_state:
    st.session_state['init_followup_clean_done'] = True
    defaults = {
        "facility_name": "選択してください", "patient_id": "", "reporter_email": "",
        "report_cycle": "6ヶ月目報告", 
        "tp_data": {
            "early": {"eval_date": None, "cd": "選択してください", "cyto": "選択してください", "treat": "選択してください"},
            "late": {"eval_date": None, "cd": "選択してください", "cyto": "選択してください", "treat": "選択してください"}
        },
...
def send_email(report_content, pid, facility, reporter_email=None):
    try:
        ...
        msg['Subject'] = f"【JUOG フォローアップ】（{facility} / ID: {pid}）"
...
with col_h3:
    cycles = ["6ヶ月目報告", "12ヶ月目報告", "18ヶ月目報告", "24ヶ月目報告"]
    st.session_state.report_cycle = st.selectbox("報告サイクル*", cycles, index=get_idx(cycles, st.session_state.report_cycle))

# ラベル設定
if st.session_state.report_cycle == "6ヶ月目報告": t1, t2 = "3ヶ月時点", "6ヶ月時点"
elif st.session_state.report_cycle == "12ヶ月目報告": t1, t2 = "9ヶ月時点", "12ヶ月時点"
elif st.session_state.report_cycle == "18ヶ月目報告": t1, t2 = "15ヶ月時点", "18ヶ月時点"
...
def render_tp(tp_key, label):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.tp_data[tp_key]["eval_date"] = st.date_input(f"{label} 来院日*", value=st.session_state.tp_data[tp_key]["eval_date"], key=f"d_{tp_key}")
        cd_opts = ["選択してください", "Grade 0", "Grade I", "Grade II", "Grade III", "Grade IV", "Grade V"]
        st.session_state.tp_data[tp_key]["cd"] = st.selectbox("合併症 (CD分類)*", cd_opts, index=get_idx(cd_opts, st.session_state.tp_data[tp_key]["cd"]), help=HELP_CD, key=f"c_{tp_key}")
    with c2:
        cy_opts = ["選択してください", "Negative", "AUC", "SHGUC", "HGUC", "LGUC", "判定不能", "未実施"]
        st.session_state.tp_data[tp_key]["cyto"] = st.selectbox("尿細胞診*", cy_opts, index=get_idx(cy_opts, st.session_state.tp_data[tp_key]["cyto"]), key=f"cy_{tp_key}")
        tx_opts = ["選択してください", "無治療", "EVP継続", "ペムブロ維持", "ニボ単剤", "化学療法", "その他"]
        st.session_state.tp_data[tp_key]["treat"] = st.selectbox("治療状況*", tx_opts, index=get_idx(tx_opts, st.session_state.tp_data[tp_key]["treat"]), key=f"t_{tp_key}")

    st.markdown("**血液検査データ**")
    bc1, bc2 = st.columns(2)
    # Na, K, Cl, ALP, TP, BUNを除去
    labs = [
    ...
    for i, (name, unit, step) in enumerate(labs):
        ...
        k = f"lab_{tp_key}_{name}"
        if k not in st.session_state: st.session_state[k] = None
        st.session_state[k] = col.number_input(f"{name} ({unit})", value=st.session_state[k], step=step, key=k)
...
with tab3:
    ...
    with sc1:
        ...
        if st.session_state.status_alive == "生存":
            ...
            st.session_state.death_cause = st.selectbox("死因*", d_opts, index=get_idx(d_opts, st.session_state.death_cause))
    with sc2:
        ...
        if st.session_state.pfs_status == "あり":
            ...
            st.session_state.pfs_site = st.multiselect("部位*", ["膀胱内", "局所", "リンパ節", "遠隔転移", "その他"], default=st.session_state.pfs_site)
    ...
    if st.button("🚀 確定送信", type="primary", use_container_width=True):
        ...
        if d.status_alive == "死亡" and d.tp_data["late"]["cd"] != "Grade V": errors.append("・死亡なのにCD分類が不整合です")
        ...
        if errors: st.error("修正してください：\n" + "\n".join(errors))
            ...
            rep += f"--- {t1}の経過 ---\n評価日: {d.tp_data['early']['eval_date']} / WBC: {fn(d.lab_early_WBC)} / Cre: {fn(d.lab_early_Cre)}\n"
            rep += f"--- {t2}の経過 ---\n評価日: {d.tp_data['late']['eval_date']} / WBC: {fn(d.lab_late_WBC)} / Cre: {fn(d.lab_late_Cre)}\n"
            rep += f"--- 状況 ---\n生存: {d.status_alive} / 再発: {d.pfs_status}"
            
            if send_email(rep, d.patient_id, d.facility_name, d.reporter_email):
                st.success(f"{d.reporter_email} 宛に控えを送信しました。"); st.balloons()
