...
- Grade I：薬物・外科的治療等を要さない正常な術後経過からの逸脱。
- Grade II：制吐・解熱・鎮痛剤以外の薬物療法（輸血、中心静脈栄養含む）を要するもの。
...
# --- セッション状態初期化 ---
if 'init_clinical_followup_v2_done' not in st.session_state:
    st.session_state['init_clinical_followup_v2_done'] = True
    # 採血項目（分画含む）
...
with c3:
    timing_map = {"3ヶ月後": 3, "6ヶ月後": 6, "9ヶ月後": 9, "12ヶ月後": 12, "15ヶ月後": 15, "18ヶ月後": 18, "21ヶ月後": 21, "24ヶ月後": 24}
    timings = ["選択してください"] + list(timing_map.keys()) + ["その他"]
    ...
    if st.session_state.op_date and st.session_state.report_timing in timing_map:
        months = timing_map[st.session_state.report_timing]
        target_date = st.session_state.op_date + timedelta(days=30 * months)
        st.info(f"💡 報告目安日: {target_date.strftime('%Y年%m月%d日')} 前後")

# --- 2. 必須項目：予後（再発が左、生存が右） ---
st.markdown('<div class="juog-header">2. 予後・診察日</div>', unsafe_allow_html=True)
p1, p2 = st.columns(2)
with p1:
    # 再発（左）
...
# --- 3. 治療状況 (生存時のみ表示) ---
if st.session_state.status_alive == "生存":
    st.markdown('<div class="juog-header">3. 現在の治療状況</div>', unsafe_allow_html=True)
...
# --- 4. 随時入力：合併症・有害事象 ---
st.markdown('<div class="juog-header">4. イベント報告（合併症・有害事象）</div>', unsafe_allow_html=True)
has_event = st.checkbox("合併症、または特記すべき有害事象（副作用）がある", value=(True if st.session_state.cd_grade != "Grade 0" or st.session_state.ae_status != "なし" else False))
if has_event:
    e1, e2 = st.columns(2)
    with e1:
...
# --- 5. 採血検査データ (12ヶ月目は必須表示) ---
is_12m = (st.session_state.report_timing == "12ヶ月後")
st.markdown(f'<div class="juog-header">5. 採血検査データ {"(12ヶ月目：必須)" if is_12m else "(実施時のみ)"}</div>', unsafe_allow_html=True)

if is_12m: st.info("ℹ️ 12ヶ月目報告のため、白血球分画を含む全ての項目を入力してください。")
show_labs = st.checkbox("採血結果を入力する", value=is_12m)
...
if st.button("🚀 事務局へ確定送信", type="primary", use_container_width=True):
    ...
    if not d.op_date: errors.append("・手術日")
    ...
    if is_12m and not show_labs: errors.append("・12ヶ月目報告には採血データの入力が必要です")
    if is_12m and d.lab_Neutro == 0: errors.append("・12ヶ月目報告には白血球分画の入力が必要です")
    ...
    if errors:
        ...
        rep = f"""【JUOG 定期報告】
時期: {d.report_timing} / ID: {d.patient_id} / 手術日: {d.op_date}
生存: {d.status_alive} / 再発: {d.pfs_status}
治療: {d.treat_status if d.status_alive=='生存' else '死亡につき無'}
